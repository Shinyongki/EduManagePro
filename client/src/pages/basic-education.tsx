...
export default function BasicEducationPage() {
  ...
  const handleUpload = async () => {
    if (!uploadedFile) {
      toast({
        title: "파일을 선택해주세요",
        description: "업로드할 기본 교육 데이터 파일을 선택해주세요.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "업로드 완료",
        description: `${result.count}개의 기본 교육 데이터가 성공적으로 업로드되었습니다.`,
      });
      
      // 파일 선택 초기화
      setUploadedFile(null);
      const fileInput = document.getElementById('basic-education-file') as HTMLInputElement;
      if (fileInput) fileInput.value = '';
      
      // 서버에서 IndexedDB로 자동 동기화
      console.log('🔄 서버 → IndexedDB 자동 동기화 시작...');
      const syncResponse = await fetch('/api/education/basic');
      if (syncResponse.ok) {
        const syncData = await syncResponse.json();
        const { IndexedDBStorage } = await import('@/lib/indexeddb');
        const educationDB = new IndexedDBStorage();
        await educationDB.setItem('basicEducationData', syncData);
        setBasicEducationData(syncData);
        console.log(`✅ IndexedDB 동기화 완료: ${syncData.length}개 기본교육 데이터`);
      }
        ...
        <TabsContent value="upload" className="mt-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Excel 파일을 통해 기본 교육 수료 데이터를 일괄 업로드합니다
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  업로드 파일은 다음 헤더를 포함해야 합니다: 이름, 기관, 과정명, 상태, 수료일 등
                </AlertDescription>
              </Alert>

              <div className="space-y-2">
                <Label htmlFor="basic-education-file">파일 선택</Label>
                <Input
                  id="basic-education-file"
                  type="file"
                  accept=".xlsx,.xls,.csv"
